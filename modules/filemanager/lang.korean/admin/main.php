...
<div id="catebody">
	...
	?>
		...
		<?php $IM=getimagessize($pwd.getUTFtoKR($file))?>
		...
		<table class="ftable">
			...
			<?php $IM=array();if(strstr('jpeg,jpg,gif,png,swf',strtolower($file_ext)))$IM=getimagessize($tdir.$files[$i])?>

			<tr class="loop">
				<td><input type="checkbox" name="members[]" value="<?php echo getKRtoUTF($files[$i])?>" /></td>
				<td><?php echo ($filenum-$i)?></td>
				<td class="code">
					<img src="<?php echo $g['img_core']?>/file/small/<?php echo is_file($g['path_core'].'images/file/small/'.$file_ext.'.gif')?$file_ext:'unknown'?>.gif" alt="<?php echo $file_ext?>" />
